...
export default function UserNewsFeed({ navigation, route }) {
  const [filterUser, filterSetUser] = useState(route.params.user);
      ...
      try {
        ...
        if (user) {
          ...
          const filteredCatalogData = catalogData.filter(
            (catalog) => catalog.userId == filterUser
          );
          setCatalogs(filteredCatalogData);
        } else {
              ...
              >
                ...
                <Card.Content>
                  <RNPText variant="labelMedium">
                    ...
                    {/* Ensure this accesses the correct field */}
                  </RNPText>
                  <RNPText variant="labelMedium">
